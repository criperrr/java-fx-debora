...
public class MainController {
  ...
  public void initialize() {
    ...
    // configurar placeholders nos campos
    txtName.setPromptText("Digite o nome do produto");
    txtDescription.setPromptText("Digite a descrição (opcional)");
    txtPrice.setPromptText("Digite o preço (ex: 10.50)");
    txtSearch.setPromptText("Buscar por nome, descrição ou ID");

    // configurar ordem de navegação TAB
    txtName.setStyle("-fx-focus-color: -fx-text-box-border;");
    txtDescription.setStyle("-fx-focus-color: -fx-text-box-border;");
    txtPrice.setStyle("-fx-focus-color: -fx-text-box-border;");
      ...
      protected void updateItem(String price, boolean empty) {
        ...
        if (empty || price == null) {
          ...
          try {
            ...
            showAlert(Alert.AlertType.WARNING, "Você deve inserir um numero no formato 00.00 ou 0");
          }
        }
      }
    });
    ...
    tableItems.getSelectionModel().selectedItemProperty().addListener((obs, old, selected) -> {
      if (selected != null) {
        ...
        btnDelete.setDisable(false); // habilitar botao delete quando item selecionado
      } else {
        btnDelete.setDisable(true); // desabilitar se nenhum item selecionado
      }
    });
    ...
    // listener para habilitar/desabilitar botao save conforme campos
  ...
  private void updateButtonStates() {
    ...
    boolean hasPrice = !txtPrice.getText().trim().isEmpty();
    boolean isSelected = tableItems.getSelectionModel().getSelectedItem() != null;

    btnSave.setDisable(!(hasName && hasPrice));
  ...
  void onSave(ActionEvent event) {
    ...
    if (name.isEmpty()) {
      showAlert(Alert.AlertType.ERROR, "nome e obrigatorio");
      return;
    }
    if (priceStr.isEmpty()) {
      showAlert(Alert.AlertType.ERROR, "preco e obrigatorio");
      return;
    }
    ...
    try {
      ...
      showAlert(Alert.AlertType.ERROR, " preco invalido - você deve inserir um numero no formato 00.00 ou 0");
      return;
    }
    ...
    try {
      ...
      if ((!idStr.isEmpty()) & Utils.isInteger(idStr)) {
  ...
  void onClear(ActionEvent event) {
    txtId.setText("selecione um item!");
    txtName.clear();
