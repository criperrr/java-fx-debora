...
public class DatabaseConnection {
    ...
    public static void resetSpongeBobDatabase() {
        try (Connection conn = DriverManager.getConnection(url, user, password);
             Statement stmt = conn.createStatement()) {
            stmt.execute("TRUNCATE TABLE spongebob_items RESTART IDENTITY;");
        } catch (SQLException e) {
            throw new RuntimeException("Erro ao resetar banco do Bob Esponja: " + e.getMessage(), e);
        }
    }
